...
/**
 * Manages block names and data by loading and categorizing block information from JSON
 * configuration files. This class facilitates filtering and retrieving block data based on
...
public class BlockNameManager extends JsonManager {
   ...
   * @param category   The category to filter blocks by. If null, empty, or "All", blocks from all
  ...
  /**
   * Checks whether a block belongs to a specified category based on the categories listed in a JSON
   * array. This method supports filtering blocks by a specific category or including all if "All"
   * is specified.
   *
   ...
   * @return true if the block's categories include the specified category or if "All" is specified,
   * false otherwise.
   */
