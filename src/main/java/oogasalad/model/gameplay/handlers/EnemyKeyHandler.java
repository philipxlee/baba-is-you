...
//        logger.info("Created enemy handler for artificial player");
  ...
  /**
   * Moves all enemy entities towards the nearest controllable block (player) on the grid. If no
   * enemy entities exist or if no controllable block is found, the method returns without
   * performing any action. If an enemy entity overlaps with a controllable block, the block is
   * removed from the grid. Enemy entities are moved one step closer to the nearest controllable
   * block based on the shortest path algorithm. If a valid path to the controllable block exists,
   * the enemy entity is moved accordingly. This method also notifies the grid observer after all
   * enemy entities have been moved.
   */
   ...
   * @return Optional containing the coordinates of the nearest controllable block if found,
   * otherwise empty.
   */
  ...
  /**
   * Sorts a list of coordinates based on their distances to a specified enemy camp using a custom
   * comparator.
   *
  ...
  /**
   * Finds the shortest path from a starting point to a target point using breadth-first search
   * (BFS).
   *
   * @param start  The starting point coordinates.
   * @param target The target point coordinates.
   * @return Optional containing the shortest path as a list of coordinates if found, otherwise
   * empty.
   */
  ...
  /**
   * Determines the shortest path from the start coordinates to the target coordinates using a
   ...
   * @return An optional containing the shortest path as a list of coordinates if found, or empty if
   * no path exists.
   */
  private Optional<List<int[]>> determinePath(int[] start, int[] target, Queue<int[]> queue,
      int[][] distances, int numRows, int numCols, boolean[][] visited) {
    while (!queue.isEmpty()) {
   ...
   */
  private void evaluateNeighbors(int row, int col, int numRows, int numCols, int[][] distances,
      boolean[][] visited, Queue<int[]> queue) {
    getNeighbors(row, col, numRows, numCols, DIRECTIONS, distances)
        .stream()
        .filter(neighbor -> !visited[neighbor[0]][neighbor[1]] && grid.isPassable(neighbor[0],
            neighbor[1]))
        .forEach(neighbor -> {
  ...
  /**
   * Performs a breadth-first search to reconstruct the shortest path from the target position to
   * the start position.
   *
   ...
   */
  private void breadthSearch(int[] start, int[][] distances, int[] current,
      LinkedList<int[]> path) {
    while (!Arrays.equals(current, start)) {
      ...
      for (int[] direction : DIRECTIONS) {
        ...
        if (newRow >= 0 && newRow < distances.length && newCol >= 0
            && newCol < distances[0].length) {
   ...
   */
  private List<int[]> getNeighbors(int row, int col, int numRows, int numCols, int[][] directions,
      int[][] distances) {
